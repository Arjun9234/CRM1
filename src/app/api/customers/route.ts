...
export async function GET() {
  ...
  // Merge Firebase data with dummy data
  const combinedCustomersMap = new Map<string, Customer>();

  // Add dummy customers first
  dummyCustomers.forEach(customer => {
    combinedCustomersMap.set(customer.id, JSON.parse(JSON.stringify(customer))); // Use deep copies of dummies
  });

  // Add/overwrite with Firebase customers
